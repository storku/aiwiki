---
title: "Terms"
slug: "terms"
---*See also: [Models](/wiki/models), [Applications](/wiki/applications) and [Guides](/wiki/guides)*
## Artificial Intelligence Terms

*See also: [artificial intelligence terms](/wiki/artificial_intelligence_terms)*
**[Agentic Context Engineering](/wiki/agentic_context_engineering)**

**[AI Agents](/index.php?title=AI_Agents&action=edit&redlink=1)**

**[AI bubble](/index.php?title=AI_bubble&action=edit&redlink=1)**

**[AI Monarchy](/wiki/ai_monarchy)**

**[AI Project Management](/wiki/ai_project_management)**

**[Alibaba Cloud](/wiki/alibaba_cloud)**

**[Anthropic](/wiki/anthropic)**

**[Art](/wiki/art)**

**[Artificial general intelligence](/wiki/artificial_general_intelligence) (AGI)**

**[Artificial Superintelligence](/index.php?title=Artificial_Superintelligence&action=edit&redlink=1) (ASI)**

**[AudioCraft](/wiki/audiocraft)**

**[Backdooring LLMs](/wiki/backdooring_llms)**

**[Baidu AI](/wiki/baidu_ai)**

**[Bard](/wiki/bard)**

**[Benchmarks](/wiki/benchmarks)**

**[BERT](/wiki/bert)**

**[Browser-use agent](/wiki/browser-use_agent) (BUA)**

**[Burstiness](/wiki/burstiness)**

**[ByteDance Seed](/wiki/bytedance_seed)**

**[Claude](/wiki/claude)**

**[Claude Code](/wiki/claude_code)**

**[Clade Skills](/index.php?title=Clade_Skills&action=edit&redlink=1)**

**[Claude --dangerously-skip-permissions](/wiki/claude_--dangerously-skip-permissions)**

**[Completion](/index.php?title=Completion&action=edit&redlink=1)** - new text generated (output) by the [LLM](/wiki/llm) after you [inference](/wiki/inference) it with the [prompt](/wiki/prompt)

**[Context window](/wiki/context_window)**

**[contexts optical compression](/index.php?title=Contexts_optical_compression&action=edit&redlink=1)** - [DeepSeek-OCR](/wiki/deepseek-ocr)

**[Controversies](/index.php?title=Controversies&action=edit&redlink=1)**

**[CodeGen](/index.php?title=CodeGen&action=edit&redlink=1)**

**[Computer-use agent](/wiki/computer-use_agent) (CUA)**

**[Computer-use model](/wiki/computer-use_model)**

**[ControlNet](/index.php?title=ControlNet&action=edit&redlink=1)**

**[Copyright](/index.php?title=Copyright&action=edit&redlink=1)**

**[CUDA](/wiki/cuda)**

**[cuDNN](/wiki/cudnn)**

**[Cursor Rules](/wiki/cursor_rules)**

**[Custom GPTs](/wiki/custom_gpts)**

**[Dark Factory](/wiki/dark_factory)**

**[Dead internet theory](/index.php?title=Dead_internet_theory&action=edit&redlink=1)**

**[DeepSeek](/wiki/deepseek)**

**[DeepSeek-OCR](/wiki/deepseek-ocr)**

**[Deep Research](/index.php?title=Deep_Research&action=edit&redlink=1)**

**[Diffusion Models](/wiki/diffusion_models)**

**[DreamStudio](/index.php?title=DreamStudio&action=edit&redlink=1)**

**[Edge AI](/index.php?title=Edge_AI&action=edit&redlink=1)**

**[Emergent Abilities](/index.php?title=Emergent_Abilities&action=edit&redlink=1)** ([Emergent Abilities of Large Language Models](/index.php?title=Emergent_Abilities_of_Large_Language_Models&action=edit&redlink=1))

**[Ethics](/index.php?title=Ethics&action=edit&redlink=1)**

**[Few shot](/index.php?title=Few_shot&action=edit&redlink=1)**

**[Foundation models](/wiki/foundation_models)**

**[Frontier labs](/index.php?title=Frontier_labs&action=edit&redlink=1)**

**[Frontier models](/wiki/frontier_models)**

**[GLM](/index.php?title=GLM&action=edit&redlink=1)**

**[GPT](/wiki/gpt)**

**[GPT-1](/wiki/gpt-1)**

**[GPT-2](/wiki/gpt-2)**

**[GPT-3](/wiki/gpt-3)**

**[GPT-4](/wiki/gpt-4)**

**[Google DeepMind](/wiki/google_deepmind)**

**[Grok](/wiki/grok)**

**[Grok 3 Jailbreak](/wiki/grok_3_jailbreak)**

**[hallucination](/wiki/hallucination)**

**[HuggingGPT](/index.php?title=HuggingGPT&action=edit&redlink=1)**

**[image generator](/index.php?title=Image_generator&action=edit&redlink=1)**

**[inclusionAI](/wiki/inclusionai)**

**[inference](/wiki/inference)**

**[Isaac GR00T](/wiki/isaac_gr00t)**

**[Knowledge distillation](/wiki/knowledge_distillation)**

**[LaMDA](/wiki/lamda)**

**[LangChain](/wiki/langchain)**

**[Large language models](/index.php?title=Large_language_models&action=edit&redlink=1) ([LLMs](/wiki/llm))**

**[Large language models ranking](/wiki/large_language_models_ranking)**

**[Leaderboards](/index.php?title=Leaderboards&action=edit&redlink=1)**

**[LLM Anxiety](/wiki/llm_anxiety)**

**[LongCat-Video](/index.php?title=LongCat-Video&action=edit&redlink=1)**

**[LoRA](/wiki/lora) ([low-rank adaptation](/wiki/low-rank_adaptation))**

**[Manipulation problem](/wiki/manipulation_problem)**

**[Manus AI](/wiki/manus_ai)**

**[Meta AI](/wiki/meta_ai)**

**[Meta AI (Company)](/wiki/meta_ai_company)**

**[Meta Prompting](/wiki/meta_prompting)**

**[MiniMax](/wiki/minimax)**

**[Minimum Viable Agent](/wiki/minimum_viable_agent)**

**[Mistral AI](/wiki/mistral_ai)**

**[Mixture of Experts](/wiki/mixture_of_experts) (MoE)**

**[Model collapse](/index.php?title=Model_collapse&action=edit&redlink=1)**

**[Model hubs](/wiki/model_hubs)**

**[Moonshot AI](/wiki/moonshot_ai)**

**[Multi-head Latent Attention](/wiki/multi-head_latent_attention) (MLA)**

**[Muon optimizer](/index.php?title=Muon_optimizer&action=edit&redlink=1)**

**[Neural network](/wiki/neural_network)**

**[NVIDIA](/wiki/nvidia)**

**[NVIDIA DGX Spark](/wiki/nvidia_dgx_spark)**

**[NVIDIA Omniverse](/wiki/nvidia_omniverse)**

**[OCR Models](/wiki/ocr_models)**

**[Ollama](/wiki/ollama)**

**[One shot](/index.php?title=One_shot&action=edit&redlink=1)**

**[OpenAI](/wiki/openai)**

**[OpenAI Gym](/wiki/openai_gym)**

**[OpenAI Gym Retro](/wiki/openai_gym_retro)**

**[OpenAI Universe](/wiki/openai_universe)**

**[OpenMule](/wiki/openmule)**

**[OpenRouter](/wiki/openrouter)**

**[Paper2Video](/wiki/paper2video)**

**[Perplexity](/wiki/perplexity)**

**[PoisonGPT](/wiki/poisongpt)**

**[Post-training](/wiki/post-training)**

**[Pre-training](/wiki/pre-training)**

**[Presentations](/wiki/presentations)**

**[Prompt](/wiki/prompt)**

**[Prompt engineering](/wiki/prompt_engineering)**

**[Prompt engineering for image generation](/wiki/prompt_engineering_for_image_generation)**

**[Prompt engineering for text generation](/wiki/prompt_engineering_for_text_generation)**

**[Prompt extraction](/index.php?title=Prompt_extraction&action=edit&redlink=1)**

**[Prompt injection](/wiki/prompt_injection)**

**[Pruning](/wiki/pruning)**

**[Purple Llama](/wiki/purple_llama)**

**[PyTorch](/index.php?title=PyTorch&action=edit&redlink=1)**

**[Q* OpenAI](/wiki/q_openai)**

**[QPU](/wiki/qpu)**

**[Quantization](/wiki/quantization)**

**[Qwen](/wiki/qwen)**

**[ReasoningBank](/wiki/reasoningbank)**

**[Reinforcement learning](/wiki/reinforcement_learning)**

**[Reflection AI](/wiki/reflection_ai)**

**[RLHF](/wiki/rlhf) ([Reinforcement learning from human feedback](/wiki/reinforcement_learning_from_human_feedback))**

**[RSI](/wiki/rsi) ([Recursive self-improvement](/wiki/recursive_self-improvement))**

**[Segment Anything](/wiki/segment_anything)** - cut out any object in an image, by Meta

**[SEO for ChatGPT](/index.php?title=SEO_for_ChatGPT&action=edit&redlink=1)**

**[SmolVLA](/wiki/smolvla)**

**[Stability AI](/wiki/stability_ai)**

**[Supervised fine-tuning](/wiki/supervised_fine-tuning) (SFT)**

**[System prompt](/wiki/system_prompt)**

**[Text-to-3D](/index.php?title=Text-to-3D&action=edit&redlink=1)**

**[Text-to-audio](/index.php?title=Text-to-audio&action=edit&redlink=1)**

**[Text-to-image](/index.php?title=Text-to-image&action=edit&redlink=1)**

**[Text-to-video](/index.php?title=Text-to-video&action=edit&redlink=1)**

**[The Pile](/index.php?title=The_Pile&action=edit&redlink=1)**

**[Tokens](/wiki/tokens)**

**[Transformers](/wiki/transformers)**

**[Unstable Diffusion](/wiki/unstable_diffusion)**

**[Vector database](/wiki/vector_database)**

**[Vector embeddings](/wiki/vector_embeddings)**

**[Vibe coding](/wiki/vibe_coding)**

**[Vision encoder](/index.php?title=Vision_encoder&action=edit&redlink=1)**

**[Vision Language Model](/wiki/vision_language_model) (VLM)**

**[Vision tokens](/index.php?title=Vision_tokens&action=edit&redlink=1)**

**[Vision Transformer](/index.php?title=Vision_Transformer&action=edit&redlink=1) (ViT)**

**[VLA](/wiki/vla)**

**[Wan AI](/index.php?title=Wan_AI&action=edit&redlink=1)**

**[World models](/index.php?title=World_models&action=edit&redlink=1)**

**[xAI](/wiki/xai)**

**[XPeng IRON](/wiki/xpeng_iron)**

**[Z.ai](/wiki/z_ai)**

## Machine Learning Terms

*See also: [Machine learning terms](/wiki/machine_learning_terms)*
**[Fundamentals](/wiki/machine_learning_terms_fundamentals)****
[Natural Language Processing](/wiki/machine_learning_terms_natural_language_processing) - [Language Evaluation](/wiki/machine_learning_terms_language_evaluation)****
[Fairness](/wiki/machine_learning_terms_fairness)****
[Decision Forests](/wiki/machine_learning_terms_decision_forests)****
[Reinforcement Learning](/wiki/machine_learning_terms_reinforcement_learning)****
[Computer Vision](/wiki/machine_learning_terms_computer_vision)** - **[Image Models](/wiki/machine_learning_terms_image_models)****
[Sequence Models](/wiki/machine_learning_terms_sequence_models)****
[Clustering](/wiki/machine_learning_terms_clustering)****
[Recommendation Systems](/wiki/machine_learning_terms_recommendation_systems)****
[TensorFlow](/wiki/machine_learning_terms_tensorflow)****
[Google Cloud](/wiki/machine_learning_terms_google_cloud)**

## All ML Terms

*See also: [Machine learning terms/All](/wiki/machine_learning_terms_all)*
- [A/B testing](/wiki/a_b_testing)

- [accuracy](/wiki/accuracy)

- [action](/wiki/action)

- [activation function](/wiki/activation_function)

- [active learning](/wiki/active_learning)

- [AdaGrad](/wiki/adagrad)

- [agent](/wiki/agent)

- [agglomerative clustering](/wiki/agglomerative_clustering)

- [anomaly detection](/wiki/anomaly_detection)

- [AR](/wiki/ar)

- [area under the PR curve](/wiki/area_under_the_pr_curve)

- [area under the ROC curve](/wiki/area_under_the_roc_curve)

- [artificial general intelligence](/wiki/artificial_general_intelligence)

- [artificial intelligence](/wiki/artificial_intelligence)

- [attention](/wiki/attention)

- [attribute](/wiki/attribute)

- [attribute sampling](/wiki/attribute_sampling)

- [AUC (Area under the ROC curve)](/wiki/auc_area_under_the_roc_curve)

- [augmented reality](/wiki/augmented_reality)

- [automation bias](/wiki/automation_bias)

- [average precision](/wiki/average_precision)

- [axis-aligned condition](/wiki/axis-aligned_condition)

- [backpropagation](/wiki/backpropagation)

- [bagging](/wiki/bagging)

- [bag of words](/wiki/bag_of_words)

- [baseline](/wiki/baseline)

- [batch](/wiki/batch)

- [batch normalization](/wiki/batch_normalization)

- [batch size](/wiki/batch_size)

- [Bayesian neural network](/wiki/bayesian_neural_network)

- [Bayesian optimization](/wiki/bayesian_optimization)

- [Bellman equation](/wiki/bellman_equation)

- [BERT (Bidirectional Encoder Representations from Transformers)](/wiki/bert_bidirectional_encoder_representations_from_transformers)

- [bias (ethics/fairness)](/wiki/bias_ethics_fairness)

- [bias (math) or bias term](/wiki/bias_math_or_bias_term)

- [bigram](/wiki/bigram)

- [bidirectional](/wiki/bidirectional)

- [bidirectional language model](/wiki/bidirectional_language_model)

- [binary classification](/wiki/binary_classification)

- [binary condition](/wiki/binary_condition)

- [binning](/wiki/binning)

- [BLEU (Bilingual Evaluation Understudy)](/wiki/bleu_bilingual_evaluation_understudy)

- [boosting](/wiki/boosting)

- [bounding box](/wiki/bounding_box)

- [broadcasting](/wiki/broadcasting)

- [bucketing](/wiki/bucketing)

- [calibration layer](/wiki/calibration_layer)

- [candidate generation](/wiki/candidate_generation)

- [candidate sampling](/wiki/candidate_sampling)

- [categorical data](/wiki/categorical_data)

- [causal language model](/wiki/causal_language_model)

- [centroid](/wiki/centroid)

- [centroid-based clustering](/wiki/centroid-based_clustering)

- [checkpoint](/wiki/checkpoint)

- [class](/wiki/class)

- [classification model](/wiki/classification_model)

- [classification threshold](/wiki/classification_threshold)

- [class-imbalanced dataset](/wiki/class-imbalanced_dataset)

- [clipping](/wiki/clipping)

- [Cloud TPU](/wiki/cloud_tpu)

- [clustering](/wiki/clustering)

- [co-adaptation](/wiki/co-adaptation)

- [collaborative filtering](/wiki/collaborative_filtering)

- [condition](/wiki/condition)

- [confirmation bias](/wiki/confirmation_bias)

- [confusion matrix](/wiki/confusion_matrix)

- [continuous feature](/wiki/continuous_feature)

- [convenience sampling](/wiki/convenience_sampling)

- [convergence](/wiki/convergence)

- [convex function](/wiki/convex_function)

- [convex optimization](/wiki/convex_optimization)

- [convex set](/wiki/convex_set)

- [convolution](/wiki/convolution)

- [convolutional filter](/wiki/convolutional_filter)

- [convolutional layer](/wiki/convolutional_layer)

- [convolutional neural network](/wiki/convolutional_neural_network)

- [convolutional operation](/wiki/convolutional_operation)

- [cost](/wiki/cost)

- [co-training](/wiki/co-training)

- [counterfactual fairness](/wiki/counterfactual_fairness)

- [coverage bias](/wiki/coverage_bias)

- [crash blossom](/wiki/crash_blossom)

- [critic](/wiki/critic)

- [cross-entropy](/wiki/cross-entropy)

- [cross-validation](/wiki/cross-validation)

- [data analysis](/wiki/data_analysis)

- [data augmentation](/wiki/data_augmentation)

- [DataFrame](/wiki/dataframe)

- [data parallelism](/wiki/data_parallelism)

- [data set or dataset](/wiki/data_set_or_dataset)

- [Dataset API (tf.data)](/wiki/dataset_api_tf_data)

- [decision boundary](/wiki/decision_boundary)

- [decision forest](/wiki/decision_forest)

- [decision threshold](/wiki/decision_threshold)

- [decision tree](/wiki/decision_tree)

- [deep model](/wiki/deep_model)

- [decoder](/wiki/decoder)

- [deep neural network](/wiki/deep_neural_network)

- [Deep Q-Network (DQN)](/wiki/deep_q-network_dqn)

- [demographic parity](/wiki/demographic_parity)

- [denoising](/wiki/denoising)

- [dense feature](/wiki/dense_feature)

- [dense layer](/wiki/dense_layer)

- [depth](/wiki/depth)

- [depthwise separable convolutional neural network (sepCNN)](/wiki/depthwise_separable_convolutional_neural_network_sepcnn)

- [derived label](/wiki/derived_label)

- [device](/wiki/device)

- [dimension reduction](/wiki/dimension_reduction)

- [dimensions](/wiki/dimensions)

- [discrete feature](/wiki/discrete_feature)

- [discriminative model](/wiki/discriminative_model)

- [discriminator](/wiki/discriminator)

- [disparate impact](/wiki/disparate_impact)

- [disparate treatment](/wiki/disparate_treatment)

- [divisive clustering](/wiki/divisive_clustering)

- [downsampling](/wiki/downsampling)

- [DQN](/wiki/dqn)

- [dropout regularization](/wiki/dropout_regularization)

- [dynamic](/wiki/dynamic)

- [dynamic model](/wiki/dynamic_model)

- [eager execution](/wiki/eager_execution)

- [early stopping](/wiki/early_stopping)

- [earth mover's distance (EMD)](/wiki/earth_mover_s_distance_emd)

- [embedding layer](/wiki/embedding_layer)

- [embedding space](/wiki/embedding_space)

- [embedding vector](/wiki/embedding_vector)

- [empirical risk minimization (ERM)](/wiki/empirical_risk_minimization_erm)

- [encoder](/wiki/encoder)

- [ensemble](/wiki/ensemble)

- [entropy](/wiki/entropy)

- [environment](/wiki/environment)

- [episode](/wiki/episode)

- [epoch](/wiki/epoch)

- [epsilon greedy policy](/wiki/epsilon_greedy_policy)

- [equality of opportunity](/wiki/equality_of_opportunity)

- [equalized odds](/wiki/equalized_odds)

- [Estimator](/wiki/estimator)

- [example](/wiki/example)

- [experience replay](/wiki/experience_replay)

- [experimenter's bias](/wiki/experimenter_s_bias)

- [exploding gradient problem](/wiki/exploding_gradient_problem)

- [fairness constraint](/wiki/fairness_constraint)

- [fairness metric](/wiki/fairness_metric)

- [false negative (FN)](/wiki/false_negative_fn)

- [false negative rate](/wiki/false_negative_rate)

- [false positive (FP)](/wiki/false_positive_fp)

- [false positive rate (FPR)](/wiki/false_positive_rate_fpr)

- [feature](/wiki/feature)

- [feature cross](/wiki/feature_cross)

- [feature engineering](/wiki/feature_engineering)

- [feature extraction](/wiki/feature_extraction)

- [feature importances](/wiki/feature_importances)

- [feature set](/wiki/feature_set)

- [feature spec](/wiki/feature_spec)

- [feature vector](/wiki/feature_vector)

- [federated learning](/wiki/federated_learning)

- [feedback loop](/wiki/feedback_loop)

- [feedforward neural network (FFN)](/wiki/feedforward_neural_network_ffn)

- [few-shot learning](/wiki/few-shot_learning)

- [fine tuning](/wiki/fine_tuning)

- [forget gate](/wiki/forget_gate)

- [full softmax](/wiki/full_softmax)

- [fully connected layer](/wiki/fully_connected_layer)

- [GAN](/wiki/gan)

- [generalization](/wiki/generalization)

- [generalization curve](/wiki/generalization_curve)

- [generalized linear model](/wiki/generalized_linear_model)

- [generative adversarial network (GAN)](/wiki/generative_adversarial_network_gan)

- [generative model](/wiki/generative_model)

- [generator](/wiki/generator)

- [GPT (Generative Pre-trained Transformer)](/wiki/gpt_generative_pre-trained_transformer)

- [gini impurity](/wiki/gini_impurity)

- [gradient](/wiki/gradient)

- [gradient boosting](/wiki/gradient_boosting)

- [gradient boosted (decision) trees (GBT)](/wiki/gradient_boosted_decision_trees_gbt)

- [gradient clipping](/wiki/gradient_clipping)

- [gradient descent](/wiki/gradient_descent)

- [graph](/wiki/graph)

- [graph execution](/wiki/graph_execution)

- [greedy policy](/wiki/greedy_policy)

- [ground truth](/wiki/ground_truth)

- [group attribution bias](/wiki/group_attribution_bias)

- [hallucination](/wiki/hallucination)

- [hashing](/wiki/hashing)

- [heuristic](/wiki/heuristic)

- [hidden layer](/wiki/hidden_layer)

- [hierarchical clustering](/wiki/hierarchical_clustering)

- [hinge loss](/wiki/hinge_loss)

- [holdout data](/wiki/holdout_data)

- [hyperparameter](/wiki/hyperparameter)

- [hyperplane](/wiki/hyperplane)

- [i.i.d.](/wiki/i_i_d)

- [image recognition](/wiki/image_recognition)

- [imbalanced dataset](/wiki/imbalanced_dataset)

- [implicit bias](/wiki/implicit_bias)

- [incompatibility of fairness metrics](/wiki/incompatibility_of_fairness_metrics)

- [independently and identically distributed (i.i.d)](/wiki/independently_and_identically_distributed_i_i_d)

- [individual fairness](/wiki/individual_fairness)

- [inference](/wiki/inference)

- [inference path](/wiki/inference_path)

- [information gain](/wiki/information_gain)

- [in-group bias](/wiki/in-group_bias)

- [input layer](/wiki/input_layer)

- [in-set condition](/wiki/in-set_condition)

- [instance](/wiki/instance)

- [interpretability](/wiki/interpretability)

- [inter-rater agreement](/wiki/inter-rater_agreement)

- [intersection over union (IoU)](/wiki/intersection_over_union_iou)

- [IoU](/wiki/iou)

- [item matrix](/wiki/item_matrix)

- [items](/wiki/items)

- [iteration](/wiki/iteration)

- [Keras](/wiki/keras)

- [keypoints](/wiki/keypoints)

- [Kernel Support Vector Machines (KSVMs)](/wiki/kernel_support_vector_machines_ksvms)

- [k-means](/wiki/k-means)

- [k-median](/wiki/k-median)

- [L0 regularization](/wiki/l0_regularization)

- [L1 loss](/wiki/l1_loss)

- [L1 regularization](/wiki/l1_regularization)

- [L2 loss](/wiki/l2_loss)

- [L2 regularization](/wiki/l2_regularization)

- [label](/wiki/label)

- [labeled example](/wiki/labeled_example)

- [LaMDA (Language Model for Dialogue Applications)](/wiki/lamda_language_model_for_dialogue_applications)

- [lambda](/wiki/lambda)

- [landmarks](/wiki/landmarks)

- [language model](/wiki/language_model)

- [large language model](/wiki/large_language_model)

- [layer](/wiki/layer)

- [Layers API (tf.layers)](/wiki/layers_api_tf_layers)

- [leaf](/wiki/leaf)

- [learning rate](/wiki/learning_rate)

- [least squares regression](/wiki/least_squares_regression)

- [linear model](/wiki/linear_model)

- [linear](/wiki/linear)

- [linear regression](/wiki/linear_regression)

- [logistic regression](/wiki/logistic_regression)

- [logits](/wiki/logits)

- [Log Loss](/wiki/log_loss)

- [log-odds](/wiki/log-odds)

- [Long Short-Term Memory (LSTM)](/wiki/long_short-term_memory_lstm)

- [loss](/wiki/loss)

- [loss curve](/wiki/loss_curve)

- [loss function](/wiki/loss_function)

- [loss surface](/wiki/loss_surface)

- [LSTM](/wiki/lstm)

- [machine learning](/wiki/machine_learning)

- [majority class](/wiki/majority_class)

- [Markov decision process (MDP)](/wiki/markov_decision_process_mdp)

- [Markov property](/wiki/markov_property)

- [masked language model](/wiki/masked_language_model)

- [matplotlib](/wiki/matplotlib)

- [matrix factorization](/wiki/matrix_factorization)

- [Mean Absolute Error (MAE)](/wiki/mean_absolute_error_mae)

- [Mean Squared Error (MSE)](/wiki/mean_squared_error_mse)

- [metric](/wiki/metric)

- [meta-learning](/wiki/meta-learning)

- [Metrics API (tf.metrics)](/wiki/metrics_api_tf_metrics)

- [mini-batch](/wiki/mini-batch)

- [mini-batch stochastic gradient descent](/wiki/mini-batch_stochastic_gradient_descent)

- [minimax loss](/wiki/minimax_loss)

- [minority class](/wiki/minority_class)

- [ML](/wiki/ml)

- [MNIST](/wiki/mnist)

- [modality](/wiki/modality)

- [model](/wiki/model)

- [model capacity](/wiki/model_capacity)

- [model parallelism](/wiki/model_parallelism)

- [model training](/wiki/model_training)

- [Momentum](/wiki/momentum)

- [multi-class classification](/wiki/multi-class_classification)

- [multi-class logistic regression](/wiki/multi-class_logistic_regression)

- [multi-head self-attention](/wiki/multi-head_self-attention)

- [multimodal model](/wiki/multimodal_model)

- [multinomial classification](/wiki/multinomial_classification)

- [multinomial regression](/wiki/multinomial_regression)

- [NaN trap](/wiki/nan_trap)

- [natural language understanding](/wiki/natural_language_understanding)

- [negative class](/wiki/negative_class)

- [neural network](/wiki/neural_network)

- [neuron](/wiki/neuron)

- [N-gram](/wiki/n-gram)

- [NLU](/wiki/nlu)

- [node (neural network)](/wiki/node_neural_network)

- [node (TensorFlow graph)](/wiki/node_tensorflow_graph)

- [node (decision tree)](/wiki/node_decision_tree)

- [noise](/wiki/noise)

- [non-binary condition](/wiki/non-binary_condition)

- [nonlinear](/wiki/nonlinear)

- [non-response bias](/wiki/non-response_bias)

- [nonstationarity](/wiki/nonstationarity)

- [normalization](/wiki/normalization)

- [novelty detection](/wiki/novelty_detection)

- [numerical data](/wiki/numerical_data)

- [NumPy](/wiki/numpy)

- [objective](/wiki/objective)

- [objective function](/wiki/objective_function)

- [oblique condition](/wiki/oblique_condition)

- [offline](/wiki/offline)

- [offline inference](/wiki/offline_inference)

- [one-hot encoding](/wiki/one-hot_encoding)

- [one-shot learning](/wiki/one-shot_learning)

- [one-vs.-all](/wiki/one-vs_-all)

- [online](/wiki/online)

- [online inference](/wiki/online_inference)

- [operation (op)](/wiki/operation_op)

- [out-of-bag evaluation (OOB evaluation)](/wiki/out-of-bag_evaluation_oob_evaluation)

- [optimizer](/wiki/optimizer)

- [out-group homogeneity bias](/wiki/out-group_homogeneity_bias)

- [outlier detection](/wiki/outlier_detection)

- [outliers](/wiki/outliers)

- [output layer](/wiki/output_layer)

- [overfitting](/wiki/overfitting)

- [oversampling](/wiki/oversampling)

- [pandas](/wiki/pandas)

- [parameter](/wiki/parameter)

- [Parameter Server (PS)](/wiki/parameter_server_ps)

- [parameter update](/wiki/parameter_update)

- [partial derivative](/wiki/partial_derivative)

- [participation bias](/wiki/participation_bias)

- [partitioning strategy](/wiki/partitioning_strategy)

- [perceptron](/wiki/perceptron)

- [performance](/wiki/performance)

- [permutation variable importances](/wiki/permutation_variable_importances)

- [perplexity](/wiki/perplexity)

- [pipeline](/wiki/pipeline)

- [pipelining](/wiki/pipelining)

- [policy](/wiki/policy)

- [pooling](/wiki/pooling)

- [positive class](/wiki/positive_class)

- [post-processing](/wiki/post-processing)

- [PR AUC (area under the PR curve)](/wiki/pr_auc_area_under_the_pr_curve)

- [precision](/wiki/precision)

- [precision-recall curve](/wiki/precision-recall_curve)

- [prediction](/wiki/prediction)

- [prediction bias](/wiki/prediction_bias)

- [predictive parity](/wiki/predictive_parity)

- [predictive rate parity](/wiki/predictive_rate_parity)

- [preprocessing](/wiki/preprocessing)

- [pre-trained model](/wiki/pre-trained_model)

- [prior belief](/wiki/prior_belief)

- [probabilistic regression model](/wiki/probabilistic_regression_model)

- [proxy (sensitive attributes)](/wiki/proxy_sensitive_attributes)

- [proxy labels](/wiki/proxy_labels)

- [Q-function](/wiki/q-function)

- [Q-learning](/wiki/q-learning)

- [quantile](/wiki/quantile)

- [quantile bucketing](/wiki/quantile_bucketing)

- [quantization](/wiki/quantization)

- [queue](/wiki/queue)

- [random forest](/wiki/random_forest)

- [random policy](/wiki/random_policy)

- [ranking](/wiki/ranking)

- [rank (ordinality)](/wiki/rank_ordinality)

- [rank (Tensor)](/wiki/rank_tensor)

- [rater](/wiki/rater)

- [recall](/wiki/recall)

- [recommendation system](/wiki/recommendation_system)

- [Rectified Linear Unit (ReLU)](/wiki/rectified_linear_unit_relu)

- [recurrent neural network](/wiki/recurrent_neural_network)

- [regression model](/wiki/regression_model)

- [regularization](/wiki/regularization)

- [regularization rate](/wiki/regularization_rate)

- [reinforcement learning (RL)](/wiki/reinforcement_learning_rl)

- [ReLU](/wiki/relu)

- [replay buffer](/wiki/replay_buffer)

- [reporting bias](/wiki/reporting_bias)

- [representation](/wiki/representation)

- [re-ranking](/wiki/re-ranking)

- [return](/wiki/return)

- [reward](/wiki/reward)

- [ridge regularization](/wiki/ridge_regularization)

- [RNN](/wiki/rnn)

- [ROC (receiver operating characteristic) Curve](/wiki/roc_receiver_operating_characteristic_curve)

- [root](/wiki/root)

- [root directory](/wiki/root_directory)

- [Root Mean Squared Error (RMSE)](/wiki/root_mean_squared_error_rmse)

- [rotational invariance](/wiki/rotational_invariance)

- [sampling bias](/wiki/sampling_bias)

- [sampling with replacement](/wiki/sampling_with_replacement)

- [SavedModel](/wiki/savedmodel)

- [Saver](/wiki/saver)

- [scalar](/wiki/scalar)

- [scaling](/wiki/scaling)

- [scikit-learn](/wiki/scikit-learn)

- [scoring](/wiki/scoring)

- [selection bias](/wiki/selection_bias)

- [self-attention (also called self-attention layer)](/wiki/self-attention_also_called_self-attention_layer)

- [self-supervised learning](/wiki/self-supervised_learning)

- [self-training](/wiki/self-training)

- [semi-supervised learning](/wiki/semi-supervised_learning)

- [sensitive attribute](/wiki/sensitive_attribute)

- [sentiment analysis](/wiki/sentiment_analysis)

- [sequence model](/wiki/sequence_model)

- [sequence-to-sequence task](/wiki/sequence-to-sequence_task)

- [serving](/wiki/serving)

- [shape (Tensor)](/wiki/shape_tensor)

- [shrinkage](/wiki/shrinkage)

- [sigmoid function](/wiki/sigmoid_function)

- [similarity measure](/wiki/similarity_measure)

- [size invariance](/wiki/size_invariance)

- [sketching](/wiki/sketching)

- [softmax](/wiki/softmax)

- [sparse feature](/wiki/sparse_feature)

- [sparse representation](/wiki/sparse_representation)

- [sparse vector](/wiki/sparse_vector)

- [sparsity](/wiki/sparsity)

- [spatial pooling](/wiki/spatial_pooling)

- [split](/wiki/split)

- [splitter](/wiki/splitter)

- [squared hinge loss](/wiki/squared_hinge_loss)

- [squared loss](/wiki/squared_loss)

- [stability](/wiki/stability)

- [staged training](/wiki/staged_training)

- [state](/wiki/state)

- [state-action value function](/wiki/state-action_value_function)

- [static](/wiki/static)

- [static inference](/wiki/static_inference)

- [stationarity](/wiki/stationarity)

- [step](/wiki/step)

- [step size](/wiki/step_size)

- [stochastic gradient descent (SGD)](/wiki/stochastic_gradient_descent_sgd)

- [stride](/wiki/stride)

- [structural risk minimization (SRM)](/wiki/structural_risk_minimization_srm)

- [subsampling](/wiki/subsampling)

- [summary](/wiki/summary)

- [supervised machine learning](/wiki/supervised_machine_learning)

- [synthetic feature](/wiki/synthetic_feature)

- [tabular Q-learning](/wiki/tabular_q-learning)

- [target](/wiki/target)

- [target network](/wiki/target_network)

- [temporal data](/wiki/temporal_data)

- [Tensor](/wiki/tensor)

- [TensorBoard](/wiki/tensorboard)

- [TensorFlow](/wiki/tensorflow)

- [TensorFlow Playground](/wiki/tensorflow_playground)

- [TensorFlow Serving](/wiki/tensorflow_serving)

- [Tensor Processing Unit (TPU)](/wiki/tensor_processing_unit_tpu)

- [Tensor rank](/wiki/tensor_rank)

- [Tensor shape](/wiki/tensor_shape)

- [Tensor size](/wiki/tensor_size)

- [termination condition](/wiki/termination_condition)

- [test](/wiki/test)

- [test loss](/wiki/test_loss)

- [test set](/wiki/test_set)

- [tf.Example](/wiki/tf_example)

- [tf.keras](/wiki/tf_keras)

- [threshold (for decision trees)](/wiki/threshold_for_decision_trees)

- [time series analysis](/wiki/time_series_analysis)

- [timestep](/wiki/timestep)

- [token](/wiki/token)

- [tower](/wiki/tower)

- [TPU](/wiki/tpu)

- [TPU chip](/wiki/tpu_chip)

- [TPU device](/wiki/tpu_device)

- [TPU master](/wiki/tpu_master)

- [TPU node](/wiki/tpu_node)

- [TPU Pod](/wiki/tpu_pod)

- [TPU resource](/wiki/tpu_resource)

- [TPU slice](/wiki/tpu_slice)

- [TPU type](/wiki/tpu_type)

- [TPU worker](/wiki/tpu_worker)

- [training](/wiki/training)

- [training loss](/wiki/training_loss)

- [training-serving skew](/wiki/training-serving_skew)

- [training set](/wiki/training_set)

- [trajectory](/wiki/trajectory)

- [transfer learning](/wiki/transfer_learning)

- [Transformer](/wiki/transformer)

- [translational invariance](/wiki/translational_invariance)

- [trigram](/wiki/trigram)

- [true negative (TN)](/wiki/true_negative_tn)

- [true positive (TP)](/wiki/true_positive_tp)

- [true positive rate (TPR)](/wiki/true_positive_rate_tpr)

- [unawareness (to a sensitive attribute)](/wiki/unawareness_to_a_sensitive_attribute)

- [underfitting](/wiki/underfitting)

- [undersampling](/wiki/undersampling)

- [unidirectional](/wiki/unidirectional)

- [unidirectional language model](/wiki/unidirectional_language_model)

- [unlabeled example](/wiki/unlabeled_example)

- [unsupervised machine learning](/wiki/unsupervised_machine_learning)

- [uplift modeling](/wiki/uplift_modeling)

- [upweighting](/wiki/upweighting)

- [user matrix](/wiki/user_matrix)

- [validation](/wiki/validation)

- [validation loss](/wiki/validation_loss)

- [validation set](/wiki/validation_set)

- [vanishing gradient problem](/wiki/vanishing_gradient_problem)

- [variable importances](/wiki/variable_importances)

- [Wasserstein loss](/wiki/wasserstein_loss)

- [weight](/wiki/weight)

- [Weighted Alternating Least Squares (WALS)](/wiki/weighted_alternating_least_squares_wals)

- [weighted sum](/wiki/weighted_sum)

- [wide model](/wiki/wide_model)

- [width](/wiki/width)

- [wisdom of the crowd](/wiki/wisdom_of_the_crowd)

- [word embedding](/wiki/word_embedding)

- [Z-score normalization](/wiki/z-score_normalization)