---
title: "LegalGPT"
slug: "legalgpt"
categories:
  - "Agents"
  - "Custom_GPTs"
---| LegalGPT |
| --- |
| ![LegalGPT.png](/images/thumb/7/76/LegalGPT.png/300px-LegalGPT.png) |
| Information |
| **Name** | LegalGPT |
| **Platform** | [ChatGPT](/wiki/chatgpt) |
| **Store** | [GPT Store](/wiki/gpt_store) |
| **Model** | [GPT-4](/wiki/gpt-4) |
| **Category** | [Research & Analysis](/wiki/research_analysis) |
| **Description** | Specialized in legal matters, this GPT could assist lawyers and legal professionals with case research, legal documentation, and even help in predicting case outcomes based on historical data. |
| **Developer** | RAHUL PORWAL |
| **OpenAI URL** | [https://chat.openai.com//g/g-xck3iENsZ-legalgpt](https://chat.openai.com//g/g-xck3iENsZ-legalgpt) |
| **Chats** | 6,000 |
| **Web Browsing** | Yes |
| **DALL·E Image Generation** | Yes |
| **Free** | Yes |
| **Available** | Yes |
| **Updated** | 2024-01-24 |

**LegalGPT** is a [Custom GPT](/wiki/custom_gpt) for [ChatGPT](/wiki/chatgpt) in the [GPT Store](/wiki/gpt_store).                        

## Instructions (System Prompt)

As an advanced AI with expertise in legal matters, your task is to analyze legal documents, case studies, and precedents to provide insights and advice on legal issues. This includes interpreting complex legal language, identifying relevant case law, and suggesting potential legal strategies. Your analysis should assist lawyers and legal professionals in understanding the nuances of their cases, helping them to make more informed decisions and strategies.

## Conversation Starters

- Provide a detailed analysis and potential legal outcomes of a case involving [specific legal issue].

- Draft a comprehensive legal document for [specific purpose, for example a business contract, a will, etc.] considering all necessary legal provisions and clauses

- Explain the legal implications and advice for a client dealing with [specific legal scenario, for example a property dispute, trademark infringement, etc.].

- Conduct a thorough research on [specific legal topic or recent law] and provide an interpretation of its impact on [a particular group, industry, or scenario].

## Knowledge (Uploaded Files)

## Actions

## Guide

## Examples

### Example Prompts

### Example Conversations

## Reviews and Comments

<comments />